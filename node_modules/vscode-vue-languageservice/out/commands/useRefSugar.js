...
async function execute(ts, document, sourceFile, connection, _findReferences) {
    var _a;
    ...
    if (!scriptSetupAst)
        return;
    const genData2 = scriptSetupRanges_1.parseRefSugarRanges(ts, scriptSetupAst);
    let edits = [];
    let varsNum = 0;
    let varsCur = 0;
    ...
    for (const refCall of genData2.refCalls) {
        const left = document.getText().substring(desc.scriptSetup.loc.start + refCall.left.start, desc.scriptSetup.loc.start + refCall.left.end);
        const rightExp = refCall.rightExpression
            ? document.getText().substring(desc.scriptSetup.loc.start + refCall.rightExpression.start, desc.scriptSetup.loc.start + refCall.rightExpression.end)
            : undefined;
        const rightType = refCall.rightType
            ? document.getText().substring(desc.scriptSetup.loc.start + refCall.rightType.start, desc.scriptSetup.loc.start + refCall.rightType.end)
            : undefined;
        let right = '';
        ...
        for (const _var of refCall.vars) {
            ...
            const references = (_a = _findReferences(document.uri, varRange.start)) !== null && _a !== void 0 ? _a : [];
